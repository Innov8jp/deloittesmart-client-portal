# app.py – Client-Facing Portal for DeloitteSmart™
...
gc = gspread.Client(auth=Credentials.from_service_account_info(
    ...
    if st.button("Register"):
        if not (name and mail and comp):
            ...
            with open(fpath,'a',newline='') as f:
                writer = csv.writer(f)
                if header: writer.writerow(["timestamp","name","email","company"])
                writer.writerow([ts,name,mail,comp])
            # Log to Google Sheet
...
# --- ELIGIBILITY SELF-CHECK MODE ---
    ...
    if st.button("Calculate & Send Report"):
        ...
        pdf.ln(5)
        info = f"User:{st.session_state.user_name} | Company:{st.session_state.company} | Email:{recipient} | Score:{score}% - {status}"
